...
<div class="row">
    <div class="col-md-12">
        <section class="panel">
            <header class="panel-heading">填写品牌推广信息</header>
            <div class="panel-body">
                <form class="form-horizontal" action="../brand<#if model??>/${model.id!}</#if>.html<#if model??>?_method=PUT</#if>" method="post">
                    <#if model??>
                        <input type="hidden" name="id" value="${(model.id)!}">
                    </#if>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">品牌推广名</label>
                        <div class="col-sm-10">
                            <input class="form-control" name="name" type="text" placeholder="请输入品牌推广名"  value="${(model.name)!}">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input class="form-control" name="sort" type="text" placeholder="请输入排序" value="${(model.sort)!}">
                            <label class="error-information"></label>
                        </div>
                    </div>
                    <div class="form-group">
...
<script type="text/javascript">
    $(document).ready(function() {
        $(".btn-reset").click(function() {
            location.href = "${contextPath}/admin/brand.html";
        });
...
// 			console.log(t.$box);
        ...
        var validateFunc = function(dom){
            ...
            validator.add(name,[
                {strategy:'isNotEmpty',errorMsg:'品牌推广名不能为空!'},
            ]);
            validator.add(sort,[
                {strategy:'isNotEmpty',errorMsg:'排序不能为空!'}
            ]);
